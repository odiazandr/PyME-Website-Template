...
The verified bootstrap currently generates one static reference route and exists to prove installation, type checking, formatting, and static output. The later application layers remain planned: validated business data and configuration; explicit Astro routes and content; layouts and semantic components; design tokens and scoped styling; narrowly justified browser behavior; production validation; and deployment. Do not infer that the reference website or production gates exist merely because the runtime builds.
