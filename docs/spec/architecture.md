...
Astro-managed CSP, repository-controlled Netlify header configuration, strict source-and-artifact production validation, and Chromium browser automation are implemented and locally verified. WebKit automation is configured but environment-blocked locally; a real deployment remains a later phase. Do not infer production readiness merely because the reference site and local contracts pass.
